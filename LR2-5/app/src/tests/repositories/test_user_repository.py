...
class TestUserRepository:
    ...
    async def test_get_user_by_email(self, user_repository: UserRepository):
        user = await user_repository.create(
            UserCreate(username="Ban", email="ban@example.com", description="Writer")
        )
    ...
    async def test_update_user(self, user_repository: UserRepository):
        user = await user_repository.create(
            UserCreate(username="John", email="john@example.com", description="Tester")
        )

        updated_user = await user_repository.update(
            user.id, UserUpdate(description="Updated description")
        )
    ...
    async def test_delete_user(self, user_repository: UserRepository):
        user = await user_repository.create(
            UserCreate(
                username="Deleted", email="deleted@example.com", description="Tester"
            )
        )
    ...
    async def test_get_users(self, user_repository: UserRepository):
        first = await user_repository.create(
            UserCreate(username="First", email="first@example.com", description="First")
        )
        second = await user_repository.create(
            UserCreate(
                username="Second", email="second@example.com", description="Second"
            )
        )
