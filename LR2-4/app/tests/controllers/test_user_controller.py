...
from dto.user_dto import UserResponse, UserCreate, UserUpdate, UserListResponse
...
class TestUserController:
    ...
    @pytest.mark.asyncio
    async def test_get_user_by_id_returns_user_response(self, user_controller, mock_user_service, sample_uuid):
        mock_user = Mock()
    ...
    async def test_get_user_by_id_raises_not_found_exception_when_user_not_exists(
        ...
        with pytest.raises(NotFoundException, match=f"Пользователь с Id {sample_uuid} не найден"):
            await user_controller.get_user_by_id(mock_user_service, sample_uuid)

    @pytest.mark.asyncio
    async def test_get_all_users_returns_user_list_response(self, user_controller, mock_user_service):
        mock_users = [Mock(), Mock()]
        total_count = 2
        mock_user_service.get_by_filter.return_value = (mock_users, total_count)

        result = await user_controller.get_all_users(mock_user_service, count=10, page=0)
    ...
    @pytest.mark.asyncio
    async def test_get_all_users_with_custom_pagination_params(self, user_controller, mock_user_service):
        mock_users = [Mock()]
        total_count = 1
        mock_user_service.get_by_filter.return_value = (mock_users, total_count)
    ...
    @pytest.mark.asyncio
    async def test_create_user_returns_user_response(self, user_controller, mock_user_service):
    ...
    @pytest.mark.asyncio
    async def test_update_user_returns_user_response(self, user_controller, mock_user_service, sample_uuid):
        user_update_data = Mock(spec=UserUpdate)
        mock_user = Mock()
        mock_user_service.update.return_value = mock_user
        expected_response = UserResponse.model_validate(mock_user)

        result = await user_controller.update_user(mock_user_service, sample_uuid, user_update_data)

        assert isinstance(result, UserResponse)
        mock_user_service.update.assert_called_once_with(sample_uuid, user_update_data)

    @pytest.mark.asyncio
    async def test_delete_user_calls_service_delete_method(self, user_controller, mock_user_service, sample_uuid):
    ...
    async def test_controller_methods_call_correct_service_methods_with_parameters(
        ...
        user_update_data = Mock(spec=UserUpdate)
        mock_user_service.update.return_value = mock_user
        await user_controller.update_user(mock_user_service, sample_uuid, user_update_data)
