...
class TestProductRepository:
    ...
    async def test_update_product(self, product_repository: ProductRepository):
        product = await product_repository.create(ProductCreate(name="Apple", price=10, count=5))

        updated_product = await product_repository.update(product.id, ProductUpdate(price=15))
    ...
    async def test_get_products(self, product_repository: ProductRepository):
        first = await product_repository.create(ProductCreate(name="First", price=10, count=5))
        second = await product_repository.create(ProductCreate(name="Second", price=20, count=10))
